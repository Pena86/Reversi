...
"""Test if PyGame is installed or not
"""
try:
    import pygame
    #print (pygame)
...
class reversiGUI():
    ...
    def checkKeyPressed(self):
        if PYGAME:
            ...
            return False
        """
        #here could be something to enable user to make moves whitout pygame
        else:
            x = False
            y = False
            while 1:
                user = input("Your move or q to quit: ")
                if len(user)>0 and user[0] == 'q':
                    self.quit()
                    return True
                for l in user:
                    try:
